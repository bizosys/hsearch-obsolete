...
public class LuceneFreebaseW {
	...
	public static void main(String[] args)throws Exception {
		...
		LuceneFreebaseW fbb = new LuceneFreebaseW();
		...
		if ( fbb.threads > 1) {
			...
			for ( int i=0; i<fbb.threads; i++ ) {
				...
				LuceneTabFileCrawler tff = fbb.create(args,st, ed);
			...
			LuceneTabFileCrawler tff = fbb.create(args, fbb.startIndex,fbb.endIndex);
			tff.fetchAndIndex();
	...
	public LuceneTabFileCrawler create(String[] args, int fromIndex, int toIndex) throws Exception {
		File dir = new File(fileName);
		LuceneTabFileCrawler tw = new LuceneTabFileCrawler(dir.getAbsolutePath(),
	...
	public static class Parallalization implements Runnable {
		...
		LuceneTabFileCrawler bt = null;
		public int status = -1;
		public long duration = 0;
		
		public Parallalization(LuceneTabFileCrawler bt) {
			this.bt = bt;
		}
		public void run() {
			long start = System.currentTimeMillis();
