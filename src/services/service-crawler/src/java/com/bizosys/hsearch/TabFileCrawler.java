...
public class TabFileCrawler {
	private static Logger l = Logger.getLogger(TabFileCrawler.class.getName());
		...
		try {
			...
			while((line = reader.readLine()) !=null ) {
				...
				if ( recordsT >= betchSize ) {
					...
					if ( ! insert(records, runPlan) ) return;
					records.clear();
				} 
	    	}
			
			this.insert(records, runPlan);
			records.clear();
		} catch (Exception ex) {
			String msg = "";
	 ...
	 * @throws Exception
	 */
	private boolean insert(List<String[]> records, List<PipeIn> runPlan ) throws Exception {
